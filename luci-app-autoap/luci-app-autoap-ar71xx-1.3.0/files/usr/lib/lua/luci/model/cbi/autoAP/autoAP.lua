...
local dev = uci:get("autoAP", "general", "repeaterRadio")
...
-- Debug Level
logLevel = s:option(ListValue, "logLevel", translate("Log output level"), translate("For Debug Purpose, 'ERROR' will print ERROR Level, 'DEBUG' will print all message, 'FILE' will log in file '/tmp/autoAP.log'"))
logLevel:value(1, translate("ERROR"))
logLevel:value(2, translate("NOTICE"))
logLevel:value(3, translate("DEBUG"))
logLevel:value(4, translate("FILE"))
logLevel = s:option(DummyValue, "", translate("\n"))
--
...
repeaterRadio = s:option(ListValue, "repeaterRadio", translate("STATION Radio device"), translate("The radio device which STATION use"))
uci:foreach("wireless", "wifi-device",
			   function(s)
				  repeaterRadio:value(s['.name'])
			   end)
repeaterRadio = s:option(DummyValue, "", translate("\n"))
---

-- Interface use flag
inter = s:option(Flag, "interfaceSpecify", translate("Specify Diagnosis NIC"), translate("Whether to specify the NIC"))
inter.disabled = 0
inter = s:option(DummyValue, "", translate("\n"))
--

-- interface
interface = s:option(ListValue, "interface", translate("NIC"), translate("Program will diagnose with this NIC"))
for k, v in ipairs(luci.sys.net.devices()) do
     if v ~= "lo" then
        interface:value(v)
    end
end
interface:depends("interfaceSpecify", 1)
...
-- Package send count --
o=s:option( Value, "pkgSendCount", translate("Package send count"), translate("Diagnosis will send count package"))
o.datatype = "uinteger"
o.datatype = "max(100)"
o.rmempty = false
o = s:option(DummyValue, "", translate("\n"))
...
o=s:option( Value, "pkgLossThreshold", translate("MAX Loss package percent"), translate("Diagnosis loss package percent (unit:%) larger than this value will cause change to next STATION"))
o.datatype = "uinteger"
o.datatype = "max(100)"
o.rmempty = false
o = s:option(DummyValue, "", translate("\n"))
...
-- Average time out --
o=s:option( Value, "timeoutThreshold", translate("MAX Average time out"), translate("Diagnosis average time out (unit: ms) larger than this value will cause change to next STATION"))
o.datatype = "uinteger"
o.rmempty = false
o = s:option(DummyValue, "", translate("\n"))
--
...
-- Some Timer --
o=s:option( Value, "start", translate("Start up waiting time"), translate("First check waiting time(unit:s) after 'autoAP' launch, for system initialize"))
o.datatype = "uinteger"
o.rmempty = false
o = s:option(DummyValue, "", translate("\n"))
...
-- Cycle retry --
o=s:option( Value, "maxFailCycle", translate("Max fail cycle"), translate("autoAP will stop working after successive failures"))
o.datatype = "uinteger"
o.datatype = "max(1024)"
o.rmempty = false
o = s:option(DummyValue, "", translate("\n"))
...
-- Priority Step --
o=s:option( Value, "priorityStep", translate("Step raise priority"), translate("Dynamic raise station priority by step."))
o.datatype = "uinteger"
o.datatype = "max(1024)"
o.rmempty = false
o = s:option(DummyValue, "", translate("\n"))

-- *********** Run Initialize configure ***********
once = m:section(NamedSection, "once", "once", translate("Initialize configure"), translate("This part will automatic configure only once AFTER 'autoAP' install, But it will initialize again if 'Initialize' was selected"))
o = once:option(DummyValue, "", translate("\n"))

-- First time run
o = once:option(Flag, "init", translate("Initialize"), translate("Program will initialize configure again if was selected"))
o.disabled = 0
o = once:option(DummyValue, "", translate("\n"))

-- LAN IP Address
o=once:option( Value, "lanIP", translate("LAN IP Address"), translate("LAN IP Address"))
o.datatype="ip4addr"
o.rmempty = false
o = once:option(DummyValue, "", translate("\n"))
...
--local sl = { "nil" }
scan = once:option(Button, "_scan", translate("Scan"))
scan.inputstyle = "find"
t = m:section(Table, {}, "<abbr title=\"Wireless Local Area Network\">WLAN</abbr>-Scan", translate("Wifi networks in your local environment"))
function scan.write()
...
-- *********** relay ***********
s = m:section(TypedSection, "relay", translate("relay stations"), translate("Now maximum support 64 stations"))
s.addremove = true
s.anonymous = false
o = s:option(DummyValue, "", translate("\n"))
...
-- priority
o=s:option( Value, "priority", translate("Station priority value"), translate("Station priority, the smaller value, the higher priority."))
o.datatype = "uinteger"
o.datatype = "max(102400)"
o.rmempty = false
o = s:option(DummyValue, "", translate("\n"))
---
